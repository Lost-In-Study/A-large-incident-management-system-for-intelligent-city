...
//    public Envelope2D envelope(Polyline p,double radius){
////    	double add=distance/100*0.01;
//    	Envelope2D env_big=new Envelope2D();
//    	Envelope2D env_merge=new Envelope2D();
//    	
//    	for (int i = 2; i < p.getPointCount(); ++i){
//    		Point p1 = p.getPoint(i);
//    		Point p2 = p.getPoint(i-1);
//
//
//            env_merge.setCoords(xmin, ymin, xmax, ymax);
//    	}
// 	
//    }
//    
    ...
    public double area(List<Point> points){
    	...
    	Point leftUp=new Point(xmin,ymax);
    	Point leftBelow=new Point(xmin,ymin);
    	Point rightBelow=new Point(xmax,ymin);
    	
    	double lenX=distance(leftBelow,rightBelow);
    	double lenY=distance(leftUp,leftBelow);
    	if(lenX<1){lenX=1;}
    	if(lenY<1){lenY=1;}
...
//    	double distance=spatial.distance(new Point(114.182,22.3068),new Point(114.182,22.3069) );
